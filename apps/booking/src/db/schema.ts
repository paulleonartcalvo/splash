...
export const userSessions = pgTable(
  "user_sessions",
  {
    ...
    instanceDatetime: timestamp("instance_datetime", {
      withTimezone: true,
      mode: "string",
    }).notNull(),
  },
  (table) => [
    ...
    unique("user_sessions_unique").on(
      table.userId,
      table.sessionId,
      table.instanceDatetime
    ),
  ]
);
...
export const userOrganizations = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      .onDelete("cascade"),
    check(
      "user_organizations_role_check",
      sql`role = ANY (ARRAY[('member'::character varying)::text, ('admin'::character varying)::text, ('owner'::character varying)::text])`
    ),
    foreignKey({
      columns: [table.role],
      foreignColumns: [roles.id],
      name: "user_organizations_role_fkey",
    })
      .onUpdate("cascade")
      .onDelete("cascade"),
  ]
